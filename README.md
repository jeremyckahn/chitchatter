...
- Chitchatter is an entirely client-side communication app. It uses public Nostr servers to establish peer connections and TURN relay servers when direct peer-to-peer connections cannot be established, but there is no Chitchatter API server.
...
Some ad blockers (such as uBlock Origin) prevent connections to certain Nostr servers. This prevents Chitchatter peers from connecting. To work around this, you can either disable your ad blocker or [self-host your own Chitchatter instance](#self-hosting).
